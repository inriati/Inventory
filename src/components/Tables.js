...
import { pageVisits, kategoriAset, pageRanking } from "../data/tables";
    ...
    <Card border="light" className="shadow-sm mb-4">
      <Card.Body className="pb-0">
        <Table responsive className="table-centered table-nowrap rounded mb-0">
          ...
          <tbody>
            {kategoriAset.map(pt => <TableRow key={`kategori-aset-${pt.id}`} {...pt} />)}
          </tbody>
        </Table>
      </Card.Body>
    </Card>
...
export const InvTable = ({ inv, account, contract, deleteData }) => {
  const totalTransactions = inv.length;
  ...
  const TableRow = (props) => {
    const { nama, kategori, kondisi, lokasi, pemakai, nomorRegistrasi, owner } = props;
      ...
      <tr>
        ...
        <td>
          <Dropdown as={ButtonGroup}>
            ...
            <Dropdown.Menu>
              <Dropdown.Item>
                <Link to={{ pathname: Routes.EditData.path, state: { data: props, accounts: account, contracts: contract } }}> <FontAwesomeIcon icon={faEdit} className="me-2" /> Edit</Link>

              </Dropdown.Item>
              <Dropdown.Item className="text-danger" >
                <a href="" onClick={() => handleDelete(props.index)}><FontAwesomeIcon icon={faTrashAlt} className="me-2" /> Hapus</a>
        ...
        <Table hover className="user-table align-items-center">
          <thead>
            <tr>
              ...
              <th className="border-bottom">Nomor Registrasi</th>
              <th className="border-bottom">Kategori</th>
              <th className="border-bottom">Kondisi</th>
              <th className="border-bottom">Lokasi</th>
              <th className="border-bottom">Pemakai</th>
              <th className="border-bottom">Pemilik</th>
              <th className="border-bottom">Action</th>
            </tr>
          </thead>
          <tbody>
            {inv.map(ix => <TableRow {...ix} />)}
          </tbody>
        </Table>
        <Card.Footer className="px-3 border-0 d-lg-flex align-items-center justify-content-between">
          <Nav>
