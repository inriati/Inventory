...
export default () => {
  ...
  const { accounts, contracts } = locations.state;
  ...
  const onSubmit = async (event) => {
    ...
    for (let index = 0; index < listData.length; index++) {
      ...
      await contracts.methods.simpanData(element.nama, element.lokasi, element.kondisi, element.kategori, element.noRegister).send({ from: accounts })
    }
  ...
  const changeHandler = (event, arrayIndex) => {
    const { value, name } = event.target;
    const newData = listData.map((el, idx) => {
      if (idx === arrayIndex) {
        var xvalue = value;
        if (name == 'noRegister') {
          xvalue = value.slice(0, 4);
        }
        return {
          ...el,
          [name]: xvalue
      ...
      <Container className="px-0">
        <Row className="d-flex flex-wrap flex-md-nowrap align-items-center py-4">
          <Col className="d-block mb-4 mb-md-0">
            <h1 className="h2">Tambah Data</h1>
            {/* <p className="mb-0">
        ...
        <Form onSubmit={onSubmit}>
          {listData.map((item, index) => {
            return [
              <h6 className='h6'>Data #{index + 1}</h6>,
              <FormTambah index={index} item={item} changeHandler={changeHandler} />
            ]
          })}

          <Row>
            <Button variant="success" className="m-1" onClick={tambahData}>Tambah Baris</Button>
            &nbsp;
